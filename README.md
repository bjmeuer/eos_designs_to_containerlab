...
**eos_designs_to_containerlab** is a role to build a [containerlab](https://containerlab.srlinux.dev/) topology from [Arista AVD](https://www.avd.arista.com) project

## Requirements

- General requirements are located here: [avd-requirements](../../README.md#Requirements)
- containerlab must be installed or a host with docker or podman installed must be available in case containerized deployment will be used. See the following link for deployment models of containerlab: https://containerlab.dev/install/
- containerlab in version `>=0.25.0`
- The cEOS-lab image which should be used has to be available in the container images of the host running containerlab (Docker or Podman).
...
- `sim_configs_dir`: Location for configuration files generated by AVD. (default: '`{{ inventory_dir }}/intended/configs/`')
...
- `sim_env`: Simulation environment you want to use, currently only 'clab' is supported. (default: clab)
...
- `sim_include_avd_external_nodes`: Whether to add nodes that are not defined in the AVD fabric, ex L3 peers, server and other connected endpoints. (default: false)
...
- `containerlab_custom_interface_mapping_same_number`: If 'containerlab_custom_interface_mapping' is enabled, this knob can be used to map the EOS interface number to a meaningful linux interface number, ex. Ethernet56/1 to eth561. Per default the linux interfaces are just counted up eth1, eth2, ect. (default: true)
...
- `containerlab_set_platform`: Use platform AVD parameter for a node to set this platform in Sysdb to show it as this platform in CVP. (default: false)
...
- `containerlab_runtime`: Optional containerlab runtime backend. When set to 'podman', containerlab will use Podman instead of Docker as the container runtime. Only adds --runtime flag when defined and not 'docker'. (default: undefined, uses containerlab default which is Docker)
...
- `containerlab_token_name:` Option to override the name of the file for the CVP onboarding token which is stored in /mnt/flash/. (default: Try to read from structured_config for the device)
...
The following example shows this possibility. In this case Spine_1 should start 30 seconds after the other nodes. For this the containerlab option startup-delay (https://containerlab.dev/manual/nodes/#startup-delay) can be used.
  ...
  children:
    SIMULATION:
      ...
      vars:
          ...
          containerlab_prefix: '""'
          containerlab_runtime: podman                                   # optional: use Podman instead of Docker as container runtime
